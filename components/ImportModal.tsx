...
const ImportModal: React.FC<ImportModalProps> = ({ 
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'html' | 'json' | 'links') => {
    ...
    try {
        ...
        // 3. Category Diff
        const existingCategoryKeys = new Set(categories.map(c => c.parentId ? `${c.parentId}/${c.name}` : c.name));
  ...
  const executeImport = () => {
      ...
      if (importMode === 'merge') {
          ...
          parsedCategories.forEach(pc => {
             const key = pc.parentId ? `${pc.parentId}/${pc.name}` : pc.name;
             if (nameToIdMap.has(key)) {
                 // Map old ID -> Existing ID
                 idMap.set(pc.id, nameToIdMap.get(key)!);
             } else {
                 // It's a new category
